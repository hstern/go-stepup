...
# any failed.
#
# go.sum handling: the library has zero non-test runtime deps by design. Until a dependency
# lands, go.sum doesn't exist and the tidy check skips its drift comparison; once one lands,
# the [ -f go.sum ] branch covers it.
set -u
fail=0
